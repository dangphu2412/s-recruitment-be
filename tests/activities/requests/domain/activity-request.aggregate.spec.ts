...
describe('ActivityRequestAggregate - Performance', () => {
  describe('memory management', () => {
    it('should not leak domain events after clearing', () => {
      // Arrange
      const aggregate =
        ActivityRequestAggregateTestBuilder.aLateRequest().buildWithId(1);
      ...
      for (let i = 0; i < 10; i++) {
        aggregate.updateApprovalStatus({
          action: ApprovalRequestAction.REVISE,
          approverId: `trainer-${i}`,
          reviseNote: `Note ${i}`,
        });
      }

      expect(aggregate.getDomainEvents()).toHaveLength(10);
    ...
    it('should handle multiple rapid state changes efficiently', () => {
      ...
      // Act
      for (let i = 0; i < 100; i++) {
        aggregate.clearDomainEvents(); // Reset for next transition
        aggregate.updateApprovalStatus({
          action: ApprovalRequestAction.REVISE,
          approverId: `trainer-${i}`,
          reviseNote: `Note ${i}`,
